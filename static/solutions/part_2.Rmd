...
class_recipie <- recipe(class + class2 + class3 ~ 
                          parcel_num + total_square_footage + ASSESSED_VALUE +
                         year_built + X + Y, 
                       data=joined_classification) %>%
  update_role(parcel_num, new_role = 'PARCELID') %>%
...
```{r}
multi_metric(our_preds, truth=log10(2*ASSESSED_VALUE), estimate=.pred) %>%
  kableExtra::kable() %>%
  kableExtra::kable_material(c("striped", "hover"))
```
